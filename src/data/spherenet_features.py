"""
spherenet_features.py — verbatim port of the official SphereNet
`features.py` (dig/threedgraph/method/spherenet/features.py): the exact
sympy-generated Bessel-radial / real-spherical-harmonic-angular /
spherical-harmonic-torsion basis functions, used ONLY by
`models_3d/spherenet.py`.

REVISION NOTE: this replaces the earlier `TorsionBasisLayer` (a
hand-rolled Fourier embedding [cos(n*tau), sin(n*tau)]), which was an
honestly-documented approximation, not the official basis. `torsion_emb`
below is the real thing: a genuine product of Bessel radial functions
and real spherical harmonics evaluated at the (angle, torsion) pair,
built once at construction time via `sympy.lambdify` (matching the
official DimeNet/SphereNet convention exactly).

One numpy-API fix applied during the port: `np.math.factorial` was
removed in NumPy >= 1.25 (the `np.math` alias itself is gone); replaced
with `math.factorial` from the standard library. This is a
compatibility fix only -- the numerical formula is unchanged.
"""
import math
...
def sph_harm_prefactor(k, m):
    # REVISION: np.math.factorial -> math.factorial (np.math removed in
    # NumPy >= 1.25). See module docstring.
